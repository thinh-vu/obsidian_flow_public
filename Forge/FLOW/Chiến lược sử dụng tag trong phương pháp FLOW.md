...
1. **Thẻ linh hoạt và tạm thời**:  
    Thẻ nên được coi là **linh hoạt và nhạy với dòng thời gian** thay vì cố định. Chúng có thể đánh dấu các ghi chú tạm thời cho các nhiệm vụ hoặc giai đoạn cụ thể (ví dụ: `#review/to-review`, `#priority/medium/urgent`). Các thẻ này giúp bạn **lọc ghi chú động** mà không làm rối hệ thống.
...
| **2. Thẻ Chủ Đề/Thematic**      | `#topic/psychology`, `#topic/ai`, `#topic/wellness`                      | Bạn muốn nhóm các ghi chú theo chủ đề, bất kể chúng được lưu trữ trong thư mục nào (ví dụ: Library, Forge).          | Áp dụng thẻ chủ đề cho việc lọc nhanh chóng và tìm kiếm kiến thức theo chủ đề. Dùng cho **Library** và **Forge**.                       |
| **3. Thẻ Trạng Thái Công Việc** | `#medium`, `#waiting-for-feedback`                                       | Theo dõi trạng thái hiện tại của các ghi chú hoặc nhiệm vụ, như các bài viết đang phát triển hoặc cần đầu vào.       | Áp dụng các thẻ này để quản lý luồng công việc, đặc biệt trong **Forge**. Đây là các thẻ động và có thể được xóa hoặc cập nhật khi cần. |
| **4. Thẻ Ưu Tiên**              | `#priority/medium/urgent`, `#priority/medium`, `#priority/medium/low`    | Một số nhiệm vụ hoặc ghi chú cần được chú ý ngay lập tức hoặc có mức độ ưu tiên cao hơn.                             | Áp dụng cho các ghi chú trong **Capture** hoặc **Forge** để theo dõi mức độ ưu tiên. Dễ dàng lọc ra các công việc cần chú ý trước.      |
| **5. Thẻ Tạm Thời**             | `#review/to-review`, `#review/needs-update`,             | Ghi chú cần xem lại, cập nhật, hoặc xử lý thêm (ví dụ: nghiên cứu đã lỗi thời trong Library).                        | Sử dụng để đánh dấu các ghi chú trong **Library** hoặc **Forge** cần cập nhật. Các thẻ này có thể xóa sau khi ghi chú được cập nhật.    |
| **6. Thẻ Nguồn Tham Chiếu**     | `#source/book`, `#source/article`, `#source/interview`, `#source/report` | Bạn cần theo dõi nguồn gốc thông tin, như nghiên cứu từ sách, bài báo, hoặc phỏng vấn.                               | Áp dụng các thẻ này cho ghi chú trong **Library** để dễ dàng theo dõi nguồn gốc thông tin, đặc biệt hữu ích cho nghiên cứu.             |
...
| **8. Thẻ Hành Động**            | `#review/follow-up`, `#waiting-for-input`, `#requires-decision`          | Khi đang chờ phản hồi hoặc ghi chú cần một quyết định trước khi tiến triển tiếp.                                     | Sử dụng trong **Forge** hoặc **Capture** cho các ghi chú phụ thuộc vào hành động từ bên ngoài để tiếp tục tiến triển.                   |
| **9. Thẻ Phiên Bản Kiến Thức**  | `#version-1.0`, `#review/needs-revision`,                 | Bạn đang theo dõi các cập nhật và thay đổi đối với một ghi chú hoặc bài viết trong **Library** hoặc **Post**.        | Sử dụng để quản lý phiên bản và cải tiến nội dung, đánh dấu những gì đã hoàn thiện và những gì cần cập nhật.                            |
| **10. Thẻ Hệ Thống và Mẫu**     | `#system/template`, `#system`, `#system/config`                          | Dành cho các ghi chú hệ thống như mẫu, cấu hình, hoặc các tệp định kỳ hỗ trợ cài đặt Obsidian và quy trình làm việc. | Áp dụng trong **Vault** để phân biệt giữa các tệp hệ thống và ghi chú thông thường, giúp tìm kiếm và quản lý hệ thống dễ dàng hơn.      |
